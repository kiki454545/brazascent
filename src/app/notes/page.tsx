...
export default async function NotesPage() {
  ...
  const notes = Array.from(noteMap.entries())
    .map(([slug, name]) => ({ slug, name, count: noteCount.get(slug) || 0 }))
            ...
            {notes.length === 0 ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {notes.map(({ slug, name, count }) => (
                    <Link
                      key={slug}
                      href={`/notes/${slug}`}
                      className="group px-4 py-3 border border-border hover:border-primary transition-colors"
                    >
